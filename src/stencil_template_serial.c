 ...
 *  
 *  gcc -Iinclude src/stencil_template_serial.c -o main
 *  
 */
...
float sumV(const float *, int);

double sumM(const double *V, int N, int M);
...
int main(int argc, char **argv)
{
  ...
  for (int iter = 0; iter < Niterations; iter++) {      
      ...
      if ( output_energy_at_steps ) {
        ...
        char filename[100];
        sprintf( filename, "data/plane_%05d.bin", iter );
...
float sumV(const float *V, int N){
  float result = 0;
  for (int i = 0; i < N; i++) {
    result += V[i];
  }
