 ...
 *  └── mpirun -np 4 ./main  -n 20
 *  └── mpirun -np 4 ./main -x 250 -y 250 -n 10
 */
...
int main(int argc, char **argv)
{
  ...
  {
    printf("\r iteratio: %d", iter); fflush(stdout);
    ...
    if (neighbours[NORTH] != MPI_PROC_NULL) {
      ...
      MPI_Irecv(buffers[RECV][NORTH], sizex, MPI_DOUBLE, neighbours[NORTH], 0, myCOMM_WORLD, &reqs[r++]);
      MPI_Isend(buffers[SEND][NORTH], sizex, MPI_DOUBLE, neighbours[NORTH], 1, myCOMM_WORLD, &reqs[r++]);
    }

    if (neighbours[SOUTH] != MPI_PROC_NULL) {
      //printf("%d: %p %p\n", Rank, buffers[RECV][SOUTH], buffers[SEND][SOUTH]);
      MPI_Irecv(buffers[RECV][SOUTH], sizex, MPI_DOUBLE, neighbours[SOUTH], 1, myCOMM_WORLD, &reqs[r++]);
      MPI_Isend(buffers[SEND][SOUTH], sizex, MPI_DOUBLE, neighbours[SOUTH], 0, myCOMM_WORLD, &reqs[r++]);      
    }

    if (neighbours[EAST] != MPI_PROC_NULL) {
      //printf("%d: %p %p\n", Rank,  buffers[RECV][EAST], buffers[SEND][EAST]);
      MPI_Irecv(buffers[RECV][EAST],  sizey, MPI_DOUBLE, neighbours[EAST],  2, myCOMM_WORLD, &reqs[r++]);
      MPI_Isend(buffers[SEND][EAST],  sizey, MPI_DOUBLE, neighbours[EAST],  3, myCOMM_WORLD, &reqs[r++]);
    }

    if (neighbours[WEST] != MPI_PROC_NULL) {
      //printf("%d: %p %p\n", Rank,  buffers[RECV][WEST], buffers[SEND][WEST]);
      MPI_Irecv(buffers[RECV][WEST],  sizey, MPI_DOUBLE, neighbours[WEST],  3, myCOMM_WORLD, &reqs[r++]);
      MPI_Isend(buffers[SEND][WEST],  sizey, MPI_DOUBLE, neighbours[WEST],  2, myCOMM_WORLD, &reqs[r++]);
    }
